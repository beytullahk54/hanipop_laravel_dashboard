...
import UrunlerTema2 from "../../../components/pages_yeni/UrunlerTema2"
import LoadingComponent from "../../../components/pages_yeni/loading"
import Container from "@mui/material/Container";
import HeaderKategoriTema2 from "../../../components/pages_yeni/header/HeaderKategoriTema2";
...
const PagesYeni = () => {
    ...
    const [hasUrunler, setHasUrunler] = React.useState([]);
    const [hasKategoriler, setHasKategoriler] = React.useState([]);
        ...
        const fetchPost = async () => {
            if (query.restoran) {
                let response = await axios.post(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/urunGetir`, {
                    ...
                    setHasUrunler(res.data.hasUrunler)
                    setHasKategoriler(res.data.hasKategoriler)
                    let date1 = new Date()
                        ...
                        <>
                            ...
                            <main>
                                <Container sx={{marginTop: 8}}>
                                    <HeaderKategoriTema2 seciliDil={seciliDil} setSeciliDil={setSeciliDil} firma={firma}
                                                    kategori={kategori} temaUrl={"/tema2"}></HeaderKategoriTema2>

                                    <SimpleDialogDemo firma={firma} seciliDil={seciliDil} setSeciliDil={setSeciliDil}
                                                      title={"Dil Seçiniz"} dialogOpen={dialogOpen}
                                                      setDialogOpen={setDialogOpen}></SimpleDialogDemo>
                                    <UrunlerTema2 hasKategoriler={hasKategoriler} hasUrunler={hasUrunler}
                                                  seciliDil={seciliDil} setSeciliDil={setSeciliDil} firma={firma}
                                                  kategori={kategori} firma={firma} temaUrl={"/tema2"}></UrunlerTema2>
